...
MIN_CARACTERES_PAGINA = (
    50  # páginas com menos texto que isso (digitalizadas) são ignoradas
)
MIN_CARACTERES_CHUNK = 100  # chunks muito pequenos são descartados
MIN_PALAVRAS_AVALIAR = (
    20  # com menos palavras que isso, o filtro de qualidade não opina
)
...
def limpar_pagina(texto):
    ...
    texto = re.sub(
        r"([a-z])-\n([a-z])", r"\1\2", texto
    )  # palavra hifenizada na quebra de linha
...
def dividir(doc, tamanho, sobreposicao):
    ...
    for linha, pagina in linhas_do_documento(doc, tamanho):
        if atual and tamanho_atual + len(linha) + 1 > tamanho:
            ...
            mantidas, soma = [], 0
            for line, p in reversed(atual):
                if soma + len(line) + 1 > sobreposicao:
                    break
                mantidas.insert(0, (line, p))
                soma += len(line) + 1
            if (
                soma + len(linha) + 1 > tamanho
            ):  # linha nova muito grande: sem sobreposição
                mantidas, soma = [], 0
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Divide os textos extraídos em chunks para o RAG."
    )
    ap.add_argument(
        "--tamanho",
        type=int,
    ...
    if not arquivos:
        print(
            f"Nenhum texto encontrado em {PASTA_TEXTOS.resolve()}. Rode antes o extrair_texto.py"
        )
        return
    ...
    print(
        f"Dividindo {len(arquivos)} documentos (qualidade mínima {args.qualidade_min:.0%})...\n"
    )
    ...
    with caminho_saida.open("w", encoding="utf-8") as saida, caminho_descartados.open(
        ...
        for caminho in arquivos:
            ...
            arquivo = (
                doc.get("arquivo")
                or caminho.relative_to(PASTA_TEXTOS).with_suffix(".pdf").as_posix()
            )
            ...
            for i, pedaco in enumerate(pedacos):
                ...
                if nota is not None and nota < args.qualidade_min:
                    ...
                    reprovados += 1
                    if exemplo_ruim is None or abs(nota - args.qualidade_min) < abs(
                        exemplo_ruim["qualidade"] - args.qualidade_min
                    ):
                        exemplo_ruim = (
                            registro  # guarda o descartado mais perto do limite
                        )
                    continue
    ...
    if tamanhos:
        print(
            f"Tamanho dos aprovados: média {sum(tamanhos) // len(tamanhos)} | "
            f"mínimo {min(tamanhos)} | máximo {max(tamanhos)} caracteres"
        )
    if notas:
        faixas = [(0.8, 0.9), (0.9, 0.95), (0.95, 1.01)]
        texto_faixas = " | ".join(
            f"{a:.0%}-{min(b, 1):.0%}: {sum(1 for n in notas if a <= n < b)}"
            for a, b in faixas
        )
    ...
    for rotulo, exemplo in [
        ("APROVADO", exemplo_ok),
        ("DESCARTADO mais perto do limite", exemplo_ruim),
    ]:
        if exemplo:
            print(f"\n--- Exemplo {rotulo} ---")
            print(
                f"id: {exemplo['id']} | páginas {exemplo['pagina_inicio']}-{exemplo['pagina_fim']} "
