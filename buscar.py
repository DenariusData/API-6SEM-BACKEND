...
def main():
    ...
    ap = argparse.ArgumentParser(description="Busca os chunks mais parecidos com uma pergunta.")
    ...
    try:
        r = requests.post(f"{OLLAMA_URL}/api/embed", json={"model": args.modelo, "input": [args.pergunta]}, timeout=120)
    except requests.ConnectionError:
        sys.exit(f"Não consegui conectar ao Ollama em {OLLAMA_URL}. Confira se ele está aberto.")
    ...
    for n, (documento, titulo, p1, p2, texto, similaridade) in enumerate(linhas, 1):
        ...
        print(textwrap.indent(textwrap.fill(" ".join(texto.split())[:450], 100), "    "))
